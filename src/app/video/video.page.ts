...
export class VideoPage implements OnInit {
  ...
  async compartir() {
    ...
    Share.share({
      ...
      url:
        'https://backup.tregional.mx/Abet6/API3.php?id=' +
        spot +
        '&us=' +
        Id +
        '&uss=' +
  ...
  async getinformation() {
    ...
    this.http
      .get(
        'https://backup.tregional.mx/AbetCloud/models/queries/app/C_getRadar.php?mf=' +
          spot.mediaRef +
          '&us=' +
          ID +
          '&u=' +
  ...
  async chart() {
    ...
    this.radarChart = new Chart('radar', {
      type: 'radar',
      data: {
        labels: ['LasEstrellasXHGA', 'XEWO', 'CanalCincoXHGUE', 'XHG'],
        datasets: [
          {
            label: spot.mediaRef,
            data: this.data,
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text:
              'Transmisión de ' + spot.mediaRef + ' del 01/01/2023 al ' + hoy,
          },
          datalabels: {
            borderWidth: 2,
            borderRadius: 100,
  ...
  async ngOnInit() {
    ...
    $('#start').html(
      'Inicio de detección: <b>' + this.video.dateStart + '</b>'
    );
    $('#end').html('Fin de detección: <b>' + this.video.dateEnd + '</b>');
    $('#cer').html(
      'Certeza: <b>' + Math.round(this.video.confidence * 100) + '%</b>'
    );
